...
refresh_security = JwtRefreshBearer(secret_key=settings.SECRET_KEY, auto_error=True)
...
class JWTAuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            credentials = await access_security(token)
            user = UserModel.filter(**credentials.subject).first()
            if user is None:
...
def get_subject(user) -> Dict[str, str]:
    subject = {"id": user.id, "phone": user.phone}
    return subject
